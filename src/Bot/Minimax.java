...
public class Minimax {
    ...
    private int nextY;
    private int mode;
    private static State root; // Trạng thái hiện tại của trò chơi
    ...
    public static String EvaluateBothSides() {
        String sentence = "";
        sentence += "Lượng giá của thế cờ: " + heuristic.evaluateState(root) +"\n";
        return sentence;
    }
    public int getNextX() {
    	return nextX;
    ...
    public Cell alphaBeta(State state) {
        ...
        for (int i = 0; i < n; i++) { // giả sử chọn một trong các phần tử trong list để tìm kiếm bước đi tối ưu
        	...
        	System.out.println("Với nước đi " + (list.get(i).getX()+1) + " " + (list.get(i).getY()+1) + ":");
        ...
        System.out.println("So luong cac nuoc di" + n);
        int x = rand.nextInt(n); // chọn ra một
        System.out.println("nuoc di da chon" + x);
